...
export default function Clients() {
  ...
  const [restartingClientId, setRestartingClientId] = useState<number | null>(null);
  ...
  const handleClientRestart = (clientId: number) => {
    setConfirmDialog({
      open: true,
      title: '重启客户端',
      message: '确定要重启该客户端吗？重启期间客户端将暂时不可用。',
      onConfirm: async () => {
        setConfirmDialog(prev => ({ ...prev, open: false }));
        setRestartingClientId(clientId);
        try {
          const response = await clientService.triggerRestart(clientId);
          if (response.success) {
            showToast('重启指令已发送，客户端正在重启...', 'success');
  ...
  const handleBatchUpdate = () => {
    const updatableCount = clients.filter(c => c.is_online && c.version).length;
    if (updatableCount === 0) {
      showToast('没有可更新的在线客户端', 'error');
      return;
    }
    setConfirmDialog({
      open: true,
      title: '批量更新客户端',
      message: `确定要更新所有在线客户端 (${updatableCount} 个) ${latestVersion ? `到最新版本 v${latestVersion}` : '到最新版本'}吗？更新后客户端将自动重启。`,
      onConfirm: async () => {
        setConfirmDialog(prev => ({ ...prev, open: false }));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          {clients.some(c => c.is_online && c.version) && (
            <button
                  ...
                  <TableRow key={client.id}>
                    ...
                    <TableCell className="whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        {client.is_online && client.version && (
                          <button
                            ...
                            className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                              client.version !== latestVersion && latestVersion
                          ...
                          >
                            {updatingClientId === client.id ? (
                              ...
                              <>
                                ...
                                {client.version !== latestVersion && latestVersion ? '更新' : '重装'}
                              </>
                            )}
                          </button>
                        )}
                        {client.is_online && (
                          <button
                            onClick={() => handleClientRestart(client.id)}
                            disabled={restartingClientId === client.id}
