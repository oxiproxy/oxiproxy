...
/// 前台运行节点（共享给 `start` 的前台分支与隐藏的 `run` 服务入口）。
#[cfg(unix)]
fn run_node_foreground(
    controller_url: String,
    token: String,
    protocol: String,
    tls_ca_cert: Option<String>,
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Command::Start {
            ...
            run_node_foreground(controller_url, token, protocol, tls_ca_cert, log_dir)?;
        }
...
fn install_systemd_service(
    ...
    let config = common::systemd::SystemdServiceConfig {
        service_name,
        description: "OxiProxy Node Server".to_string(),
        binary_path,
        args,
        working_dir,
        user,
    ...
    match cli.command {
        Command::Start {
            ...
            runtime.block_on(async {
                run_node(controller_url, token, protocol, ca_cert, log_dir).await
            })
        }
...
fn update_binary(options: common::update::UpdateOptions) -> anyhow::Result<()> {
    ...
    #[cfg(target_os = "linux")]
    println!("systemd 安装方式：sudo ./node restart（自定义服务请加 --service-name <名称>）");
